...
$(document).ready(function(){
    ...
    $("#dot_dropdown").on("change", ".dot", function(e){
        e.preventDefault();
        displayChannels( g_channel_result );
    });
...
function getChannelResultItems( channel_result, dots ){
    let items = [];
 
    channel_result?.map(channel => {
        if(dots.length > 0){
            if (dots.indexOf(channel.DOT.ID) == 0){
                items.push(channel);
                return items
            }
        }
...
function getSelectedDots(){
    var inputs = $("#dot_dropdown input");1
    var dots = [];
